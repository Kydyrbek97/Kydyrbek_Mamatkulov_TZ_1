...
# Анонимные функции - lambda (Обычные функции с одной особенностью, у них нет)
# lambda функции могут принимать сколько угодно 
# параметров, но всегда возвращают одно выражение
...
# ------------------------------------------------------------------------------------
# dict_ = {'John': 500, 'Tirion': 170_00, 'Tom': 150, 'Sanzhar': 20, 'Ayana': 100_000}

# new_dict = sorted(dict_.items(), key=lambda x: x[1], reverse=True)
# print(dict(new_dict))
# ------------------------------------------------------------------------------------

# map(function, iterable) - Применяет к каждому элементу внутри iterable
# функции который мы ей  передаем закидывая в результате данные которые 
# возвращает функция, в результате мы получаем mapobject (iterator) в котором 
# хранятся все наши данные

# Например, с помощью map  можно преобразовать все элементы 
# внутри списка. Перевести все строки в верхний регистр
...
# print(result)
# -------------------------------------------------------------------
# filter(function, iterable) -> применяет ко всем 
# элементам iterable  функцию которую мы передали и возвращает 
# filterobject (итератор) только с теми элементами, для которых 
...
# ls = ['John','one', 'two', 'list', 'makers', 'ev.25', '25', 'fanta']

# res = list(filter(lambda x: len(x) > 4, ls))
# print(res)
# -------------------------------------------------------------------

# enumerate(iterable) - она пронумеровывет каждый элемент внутри 
# iterable ее собственном индексом
...
# zip(iterables) - она соединяет каждый элемент итерируемых элементов между собой 
...
#     'sw2':['london_r2', 'Mercyside', 'Cisco', '3850', '101.255.20.10'],
# }
...
# all(iterable) - Возвращает True, если ВСЕ элементы внутри Iterable
# имеют значение True, иначе возвращает False.
...
# any - Возвращает True, хотя бы один из элементов 
# дает True
